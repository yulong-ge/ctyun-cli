...
const SETTINGS = {
  projectName: process.env.CTYUN_PROJECT_NAME ?? "<PROJECT_NAME>",
  regionNameEng: process.env.CTYUN_REGION ?? "zj-pinghu-1",
  queueName: process.env.CTYUN_QUEUE ?? "q-<PROJECT>-pool02",
  gpuModel: process.env.CTYUN_GPU_MODEL ?? "NVIDIA-A100-SXM4-80GB",
  gpuCards: Number(process.env.CTYUN_GPU_CARDS ?? 4),
  usedGpuThreshold: Number(process.env.CTYUN_USED_GPU_THRESHOLD ?? 16),
};
