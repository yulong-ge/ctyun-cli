...
export async function buildCreateFormData(client, options = {}) {
  // 默认值来自 ~/.ctyun/config / 环境变量（仓库不含内部项目名）；命令行 options 最终覆盖
  ...
  const o = {
    ...
    gpuCards: cfg.gpuCards ?? 4,
    imageName: cfg.imageName,
    researchStorageName: cfg.researchStorageName,
    researchSpaceName: cfg.researchSpaceName,
    machineName: `dev-env-${Math.random().toString(36).slice(2, 7)}`,
    localExpansionGb: cfg.localExpansionGb ?? 3950,
    localMountPath: cfg.localMountPath ?? "/research",
    localPersistence: true,
    sshShareType: "Shared",
    sshClientIps: "",
    autoStop: cfg.autoStop ?? 2,
    stopDuration: 0,
    count: 1,
    sshKeyName: cfg.sshKeyName ?? "",
    ...options,
  };
  requireConfig(o, ["projectName", "regionNameEng", "queueName", "gpuModel", "imageName"], "create ");
