...
凭据目录: ~/.ctyun/ (aksk=官方密钥, credentials=控制台账密, session/=控制台会话)`);
  ...
  program.command("queues")
    ...
    .action(async options => {
      ...
      try {
        ...
        table([
          hasRegion ? ["区域", "队列", "已用/总GPU", "空闲"] : ["资源池", "队列", "已用/总GPU", "空闲", "状态"],
          ...out.map(q => hasRegion
            ? [q.region, q.queue, `${q.usedGpu}/${q.totalGpu}`, String(q.totalGpu - q.usedGpu)]
            : [q.pool ?? "-", q.queue, `${q.usedGpu ?? "?"}/${q.totalGpu}`, q.usedGpu == null ? "?" : String(q.totalGpu - q.usedGpu), q.state ?? "-"]),
        ]);
      } finally { await closeChannel(channel); }
    });
  ...
  program.command("images")
    .description("镜像列表（默认公共框架镜像；--custom 为自定义/私有镜像）")
    ...
    .action(async (keyword, options) => {
      ...
      try {
        if (options.custom) {
          ...
          for (const i of filtered) console.log(`  ${i.imageId ?? i.id}  ${i.name}:${i.imageTag ?? ""}  ${i.state ? `[${i.state}]` : ""}`);
          return;
        }
        if (channel.mode === "official") {
          ...
          for (const pool of pools) {
            ...
            for (const region of regions) {
              ...
              table([
                ...
                ...specs.map(s => [String(s.id), `${s.quotaGpu}x ${s.gpuModel ?? ""}`, String(s.quotaCpu), `${s.quotaMem}GB`, s.sellout === "SOLDOUT" ? "❌ 是" : "否", s.specType ?? "-"]),
              ]);
            }
          }
          return;
      ...
      try {
        ...
        if (channel.mode === "official") {
          try {
            ...
            console.error(`(官方通道: ${e.message} — 改用控制台通道)`);
            await closeChannel(channel);
  ...
  program.command("job")
    .description("训练作业详情（含 podList——查日志的 podName 来源）")
  ...
  program.command("job-create")
    ...
    .action(async options => {
      ...
      try {
        ...
        console.log(`✅ 作业已创建: ${r.trainingJobId}（ctyun job <id> 查看状态）`);
      } finally { await closeChannel(channel); }
    });

  program.command("job-start")
  ...
  program.command("image-save")
    .description("保存运行中开发机为自定义镜像（异步——images --custom 轮询 state=SUCCESS）")
    .argument("<ideId>", "开发机 ID", intArg)
    ...
    .action(async (ideId, options) => {
      ...
      try {
        ...
        console.log(`✅ 已提交保存: ${options.name}:${options.tag}（${r.status?.message ?? "ok"}；images --custom 查看进度）`);
      } finally { await closeChannel(channel); }
    });

  program.command("image-set")
  ...
  program.command("storage-specs")
    .description("科研存储规格列表（regionId 从 storages 列表取）")
  ...
  program.command("storage-create")
  ...
  program.command("storage-resize")
  ...
  program.command("storage-delete")
