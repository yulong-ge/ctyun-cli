...
通道说明:
  ...
  自动选择: 有 AK/SK 走官方。以下命令仅控制台通道可用（官方 API 无对应端点/权限）:
            create / preflight / pvc / rename / whoami / raw / summary / events /
...
凭据目录: ~/.ctyun/ (aksk=官方密钥, credentials=控制台账密, session/=控制台会话, config=默认值)`);
  ...
  program.command("envs")
    .description("开发机列表（等价 ctyun env list）")
    .option("--json", "JSON 输出")
  ...
  const envCmd = program.command("env")
    .description("开发机（ctyun env <id> = 详情；子命令 list/get/start/stop/delete/rename/create）")
  ...
  envCmd.command("get")
    .description("开发机详情（等价 ctyun env <id>）")
  ...
  program.command("start")
    .description("启动开发机（仅已停止/失败/异常态；等价 ctyun env start）")
    .argument("<id>", "开发机 ID", intArg)
  ...
  program.command("stop")
    .description("停止开发机（释放 GPU，数据保留；等价 ctyun env stop）")
    .argument("<id>", "开发机 ID", intArg)
    .option("--dry-run", "只打印请求体")
    .action(envStopAction);

  program.command("delete")
    .description("删除开发机（不可恢复；等价 ctyun env delete）")
    .argument("<id>", "开发机 ID", intArg)
    .requiredOption("--yes", "确认删除（必须显式给出）")
    .action(envDeleteAction);

  program.command("rename")
    .description("修改开发机别名（空串清除；仅控制台通道；等价 ctyun env rename）")
    .argument("<id>", "开发机 ID", intArg)
    .argument("<alias>", "新别名")
    .action(envRenameAction);

  program.command("create")
    .description("创建开发机（默认=用户配置 ~/.ctyun/config；依赖控制台通道；等价 ctyun env create）")
  ...
  // env 组的其余子命令（详情 list/get 已在 env 命令定义处注册）
  envCmd.command("start")
    .description("启动开发机（等价顶层 start）")
    .argument("<id>", "开发机 ID", intArg)
    .option("--cpu-only", "仅 CPU 启动（仅控制台通道支持 startMode）")
    .option("--dry-run", "只打印请求体")
    .action(envStartAction);
  ...
  envCmd.command("stop")
    .description("停止开发机（等价顶层 stop）")
    .argument("<id>", "开发机 ID", intArg)
    .option("--dry-run", "只打印请求体")
    .action(envStopAction);

  envCmd.command("delete")
    .description("删除开发机（不可恢复；等价顶层 delete）")
    .argument("<id>", "开发机 ID", intArg)
    .requiredOption("--yes", "确认删除（必须显式给出）")
    .action(envDeleteAction);

  envCmd.command("rename")
    .description("修改别名（等价顶层 rename）")
    .argument("<id>", "开发机 ID", intArg)
    .argument("<alias>", "新别名")
    .action(envRenameAction);

  envCmd.command("create")
    .description("创建开发机（等价顶层 create；默认=用户配置 ~/.ctyun/config）")
    .option("--dry-run", "只打印提交体")
    .option("--yes", "确认提交（真实写操作）")
    .option("--projectName <name>", "企业项目名")
    .option("--region <name>", "可用区 (zj-pinghu-1)")
  ...
  const preflightAction = async options => {
    const channel = await pickChannel({ prefer: "console", force: options.channel });
    try {
  ...
  program.command("preflight")
    .description("抢卡预检：监控目标队列快照（GPU 占用/售罄/配额；目标来自 ~/.ctyun/config）〔console〕")
    .option("--json", "JSON 输出")
    .action(preflightAction);

  program.command("pool", { hidden: true })
    .description("(已废弃，改名 preflight) 抢卡预检快照")
    .option("--json", "JSON 输出")
    .action(async options => {
      console.error("(提示: pool 已改名 preflight——避免与资源池列表 pools 混淆)");
      await preflightAction(options);
    });
  ...
  program.command("images")
    .description("镜像列表（等价 ctyun image list；--custom 为自定义/私有镜像）")
  ...
  program.command("keys")
    .description("SSH 公钥（等价 ctyun key list）")
    .option("--json", "JSON 输出")
  ...
  program.command("jobs")
    .description("训练作业列表（等价 ctyun job list）")
    .option("--json", "JSON 输出")
    ...
    .action(async options => {
      ...
      try {
        ...
        if (Number(ide?.demandSummary?.totalNum ?? 1) === 0 && Number(ide?.periodSummary?.totalNum ?? 1) === 0) {
          console.log("  (口径说明: 控制台 summary 按需求单统计，本账号为 0——机器清单以 ctyun envs 为准)");
        }
  ...
  program.command("my-ip")
    ...
    .action(async options => {
      ...
      try {
        ...
        console.log(ip ? `出口 IP: ${ip}  (CIDR: ${ip}/32)` : JSON.stringify(r, null, 2));
      } finally { await closeChannel(channel); }
    });
  ...
  program.command("events")
    ...
    .action(async options => {
      ...
      try {
        ...
        table([
          ["时间", "操作", "对象", "操作者"],
          ...list.map(e => [String(e.createTime ?? "-"), e.action ?? "-", e.instanceName ?? e.object ?? "-", e.operatorName ?? e.email ?? "-"]),
        ]);
      } finally { await closeChannel(channel); }
    });
  ...
  image.command("list")
    .description("镜像列表（等价顶层 images 命令；--custom 为自定义/私有镜像）")
    .argument("[keyword]", "名称关键词过滤")
    .option("--region <r>", "可用区过滤（控制台通道）")
    .option("--custom", "自定义镜像（官方 listCustomerIdeImages / 控制台 user_image/list）")
    .action((keyword, options) => imagesAction(keyword, options));
  ...
  const HELP_GROUPS = [
    ...
    ["开发机", ["envs", "env", "start", "stop", "delete", "rename", "create", "jexec", "ssh-setup", "ssh-ips", "pvc", "metrics", "batch-start", "batch-stop"]],
    ["训练作业", ["jobs", "job"]],
    ["推理服务", ["infer"]],
    ["科研存储", ["storage", "pools"]],
    ["镜像", ["images", "image"]],
    ...
    ["监控与审计", ["preflight", "summary", "events", "whoami", "my-ip"]],
    ["逃生舱", ["api", "raw"]],
  ];
