...
// 通道矩阵: 官方可达成 env(start/stop/delete)/key/job/image/storage/env metrics/env exec/env ssh-setup
//           仅控制台 env create(space 级存储挂载)/queue probe 终审(env rename/env pvc/whoami/raw
...
// 变长参数的累加解析（Commander 签名 (value, previous)，须返回累计数组）
function intListArg(value, previous = []) {
  return previous.concat(intArg(value));
}
...
export function buildProgram() {
  ...
  program
    ...
    .option("--json", "JSON 输出")
    .option("--channel <type>", "强制通道: official | console（默认自动选择）", channelArg)
    .hook("preAction", (thisCommand, actionCommand) => {
      const merged = actionCommand.optsWithGlobals();
      for (const [k, v] of Object.entries(merged)) {
        if (actionCommand.opts()[k] === undefined) {
          actionCommand.opts()[k] = v;
        }
      }
    })
    .configureOutput({
...
通道说明:
  ...
  自动选择: 有 AK/SK 走官方。以下命令仅控制台通道可用（官方 API 无对应端点/权限）:
            env create / queue probe / env pvc / env rename / whoami / raw / summary / events /
            infer * / key list（子账号——官方 listPublicKeys 仅主账号）
...
凭据目录: ~/.ctyun/ (aksk=官方密钥, credentials=控制台账密, session/=控制台会话, config=默认值)`);
  ...
  program.command("config")
    .description("查看用户默认配置（env create/queue probe 缺省值来源；编辑 ~/.ctyun/config，环境变量 CTYUN_* 优先）")
    .action(async () => {
  ...
  const envDetailAction = async (id, options) => {
    ...
    try {
      ...
      if (info.queueName || info.queueId) console.log(`队列:     ${info.queueName ?? info.queueId}${info.queueId ? ` (ctyun queue list 查 ID 对照)` : ""}`);
  ...
  const envCmd = program.command("env")
    .description("开发机（env <id> = 详情；全部操作见子命令）")
  ...
  envCmd.command("list")
    .description("开发机列表")
    .option("--json", "JSON 输出")
    .option("--per-page <n>", "每页条数（默认 100）", intArg)
    .option("--all-projects", "不过滤企业项目（控制台通道）")
    .action(envsListAction);



  const envStartAction = async (ids, options) => {
    const idList = Array.isArray(ids) ? ids : [ids];
    if (idList.length > 1) {
      // 多台：控制台批量端点；官方通道无批量接口，逐台提交
      if (options.cpuOnly) throw new Error("--cpu-only 仅支持单台启动");
      const channel = await pickChannel({ prefer: "console", force: options.channel });
      try {
        if (options.dryRun) {
          console.log(channel.mode === "official"
            ? `[dry-run] POST /api/bc/v2/launchIde × ${idList.length}（官方无批量端点，逐台提交）`
            : "[dry-run] POST /bc/v1/ide/batchLaunch");
          console.log(JSON.stringify(channel.mode === "official" ? idList.map(id => ({ ideId: id })) : { ids: idList.map(Number) }, null, 2));
          return;
        }
        if (channel.mode === "official") {
          for (const id of idList) {
            requireOfficialOk(await channel.official.call("launchIde", { ideId: id }), `启动开发机 ${id}`);
          }
          console.log(`✅ 已提交启动 ${idList.length} 台（官方通道逐台提交）`);
          return;
        }
        const r = requireOk(await channel.console.postV1("/ide/batchLaunch", { ids: idList.map(Number) }), "批量启动");
        console.log(`✅ 已批量提交启动 ${idList.length} 台: ${r.message ?? r.status?.message ?? "ok"}`);
      } finally { await closeChannel(channel); }
      return;
    }
    const id = idList[0];
  ...
  const envStopAction = async (ids, options) => {
    const idList = Array.isArray(ids) ? ids : [ids];
    if (idList.length > 1) {
      // 多台：控制台批量端点；官方通道无批量接口，逐台提交
      const channel = await pickChannel({ prefer: "console", force: options.channel });
      try {
        if (options.dryRun) {
          console.log(channel.mode === "official"
            ? `[dry-run] POST /api/bc/v2/stopIde × ${idList.length}（官方无批量端点，逐台提交）`
            : "[dry-run] POST /bc/v1/ide/batchStop");
          console.log(JSON.stringify(channel.mode === "official" ? idList.map(id => ({ ideId: id })) : { ids: idList.map(Number) }, null, 2));
          return;
        }
        if (channel.mode === "official") {
          for (const id of idList) {
            requireOfficialOk(await channel.official.call("stopIde", { ideId: id }), `停止开发机 ${id}`);
          }
          console.log(`✅ 已提交停止 ${idList.length} 台（官方通道逐台提交）`);
          return;
        }
        const r = requireOk(await channel.console.postV1("/ide/batchStop", { ids: idList.map(Number) }), "批量停止");
        console.log(`✅ 已批量提交停止 ${idList.length} 台: ${r.message ?? r.status?.message ?? "ok"}`);
      } finally { await closeChannel(channel); }
      return;
    }
    const id = idList[0];
  ...
  const envCreateAction = async options => {
    if (options.channel === "official") {
      throw new Error("env create 依赖控制台通道（space 级科研存储挂载 + 企业项目发现，官方 createIde 仅 storageId 粒度）。请先 ctyun login");
    }
    const channel = await pickChannel({ prefer: "console", force: options.channel });
    try {
  ...
  // 开发机生命周期统一入口是 env 组子命令（env start/stop/delete/rename/create，见文末挂载）
  const withCreateOptions = cmd => cmd
    .option("--dry-run", "只打印提交体")
    .option("--yes", "确认提交（真实写操作）")
    .option("--project-name <name>", "企业项目名")
    .option("--region <name>", "可用区 (zj-pinghu-1)")
    .option("--queue <name>", "队列名")
    .option("--gpu-model <model>", "GPU 型号")
  ...
  const queuesListAction = async options => {
      const channel = await pickChannel({ prefer: "official", force: options.channel });
      try {
        const out = [];
        if (channel.mode === "official") {
  ...
  const poolSnapshotAction = async options => {
    // 默认控制台口径（创建链路的真实判定）；无会话且配了 AK/SK 时自动降级官方口径
    let channel;
    try {
      channel = await pickChannel({ prefer: "console", force: options.channel });
    } catch (e) {
      if (options.channel || !/控制台会话不存在/.test(e.message)) throw e;
      const { readAksk } = await import("./official-client.mjs");
      if (!await readAksk()) throw e;
      console.error("(控制台无会话——降级官方口径；队列级终审请先 ctyun login)");
      channel = await pickChannel({ prefer: "official", force: "official" });
    }
    try {
      const pool = await import("./pool.mjs");
      const snap = channel.mode === "official"
        ? await pool.snapshotQueueOfficial(channel.official)
        : await pool.snapshotQueue(channel.console);
      if (options.json) { console.log(JSON.stringify(snap, null, 2)); return; }
      if (snap.channel === "official") {
        console.log(`队列:   ${snap.queue} (${snap.region})  〔官方口径〕`);
        console.log(`GPU:    ${snap.usedGpu}/${snap.totalGpu} 已用, 空闲 ${snap.freeGpu}  (CPU 空闲 ${snap.cpuFree}核 / 内存空闲 ${snap.memFree}GB)`);
        console.log(`池库存: ${snap.poolSellout ?? "?"}（资源池层，①供应商库存）`);
        console.log(`可提交: ${snap.ready ? "✅ 是" : "❌ 否"}（口径: ②队列空卡≥目标卡数 + 池层未售罄 + 占用<阈值）`);
        console.log(`(队列级售罄/配额终审: ctyun queue probe --channel console)`);
        return;
      }
      console.log(`队列:   ${snap.queue} (${snap.region})  〔控制台口径〕`);
      console.log(`GPU:    ${snap.usedGpu}/${snap.totalGpu} 已用, 空闲 ${snap.freeGpu}`);
      console.log(`售罄:   ${snap.sellout === 1 ? "是" : "否"}（队列分配层）   满足需求: ${snap.meetNeed === 1 ? "是" : snap.meetNeed === 2 ? "否" : String(snap.meetNeed)}`);
  ...
  // pool 专指资源池（池是队列的容器）；原顶层 pool（队列快照）更名为 queue probe
  const poolCmd = program.command("pool")
    .description("资源池（池是队列的容器）");
  ...
  const queueCmd = program.command("queue")
    .description("队列（list = 清单与 GPU 占用；probe = 监控目标队列快照）");
  queueCmd.command("list")
    .description("队列与 GPU 占用（官方: listQueues+实例配额；控制台: 专属池队列）")
    .option("--json", "JSON 输出")
    .action(queuesListAction);
  queueCmd.command("probe")
    .description("监控目标队列快照：GPU 占用/售罄/配额/可提交（目标队列来自 ~/.ctyun/config 的 CTYUN_QUEUE）")
    .option("--json", "JSON 输出")
  ...
  const specsListAction = async options => {
      const channel = await pickChannel({ prefer: "official", force: options.channel });
      try {
        if (channel.mode === "official") {
  ...
  const envPvcAction = async (id, options) => {
      const channel = await pickChannel({ prefer: "console", force: options.channel });
      try {
        const client = channel.console;
  ...
  /** 数字开发机 ID → objectUuid（metrics 双通道共用；非数字输入原样返回） */
  const resolveObjectUuid = async (channel, idOrUuid) => {
    if (!/^\d+$/.test(String(idOrUuid))) return idOrUuid;
    const info = channel.mode === "official"
      ? await officialIdeDetail(channel.official, idOrUuid)
      : requireOk(await channel.console.getV1("/ide/get", { id: idOrUuid }), "读取开发机详情");
    const uuid = info.uuid ?? info.objectUuid ?? info.ideUuid ?? info.resourceUuid;
    if (!uuid) throw new Error(`开发机 ${idOrUuid} 详情未返回 uuid——请改传 objectUuid（ctyun env ${idOrUuid} --json 查看）`);
    return uuid;
  };

  const envMetricsAction = async (idOrUuid, options) => {
    const channel = await pickChannel({ prefer: "official", force: options.channel });
    try {
      const uuid = await resolveObjectUuid(channel, idOrUuid);
        if (channel.mode === "official") {
  ...
  const envExecAction = async (id, specParts, options) => {
      const spec = specParts.join(" ");
      if (!spec) throw new Error("用法: ctyun env exec <ideId> <python代码 | 文件.py | cmd:shell命令>");
      const { channel, info } = await ideDetail(options, id);
      try {
        let code;
  ...
  const envSshSetupAction = async (id, options) => {
      const { channel, info } = await ideDetail(options, id);
      try {
  ...
  // 批量启停已并入 start/stop 的变长参数（batch-start / batch-stop 已移除）
  ...
  program.command("events")
    .description("操作审计日志")
    .option("--per-page <n>", "条数（默认 20）", intArg)
    .option("--json", "JSON 输出")
    .action(async options => {
      const channel = await pickChannel({ prefer: "console", force: options.channel });
      try {
  ...
  job.command("list")
    .description("作业列表")
    .option("--json", "JSON 输出")
    .option("--per-page <n>", "每页条数（默认 20）", intArg)
  ...
  job.command("create")
    ...
    .requiredOption("--spec-id <id>", "规格 ID（ctyun spec list 查看）", intArg)
  ...
  job.command("logs")
    ...
    .option("--page-size <n>", "日志条数上限（官方上限 100）", intArg)
    .action(async (jobId, options) => {
      const channel = await pickChannel({ prefer: "official", force: options.channel });
      try {
  ...
  const envSetImageAction = async (ideId, options) => {
      const channel = await pickChannel({ prefer: "official", force: options.channel });
      try {
        if (channel.mode !== "official") throw new Error("env set-image 需要官方通道。先运行: ctyun aksk");
        const info = await officialIdeDetail(channel.official, ideId);
        const state = info.statesString ?? "";
  ...
  const envSshIpsAction = async (ideId, options) => {
      const channel = await pickChannel({ prefer: "official", force: options.channel });
      try {
        if (channel.mode === "official") {
  ...
  const quotasListAction = async options => {
      const channel = await pickChannel({ prefer: "official", force: options.channel });
      try {
        let data;
        if (channel.mode === "official") {
  ...
  const quotaCmd = program.command("quota")
    .description("共享集群与租户配额");
  quotaCmd.command("list")
    .description("共享集群与租户配额总览")
    .option("--json", "JSON 输出")
    .action(quotasListAction);
  ...
  // ===== env 组规范挂载（开发机全部操作的标准入口） =====
  envCmd.command("get")
    .description("开发机详情（等价 env <id>）")
    .argument("<id>", "开发机 ID", intArg)
    .option("--ssh", "只打印 SSH 命令与 Jupyter 链接")
    .option("--json", "JSON 输出")
    .action(envDetailAction);

  withCreateOptions(envCmd.command("create")
    .description("创建开发机（默认=用户配置 ~/.ctyun/config；依赖控制台通道）"))
    .action(envCreateAction);

  envCmd.command("start")
    .description("启动开发机（仅已停止/失败态；多台走批量）")
    .argument("<id...>", "开发机 ID（1 到多台）", intListArg)
    .option("--cpu-only", "仅 CPU 启动（仅控制台通道支持 startMode，仅单台）")
    .option("--dry-run", "只打印请求体")
    .action(envStartAction);

  envCmd.command("stop")
    .description("停止开发机（释放 GPU，数据保留；多台走批量）")
    .argument("<id...>", "开发机 ID（1 到多台）", intListArg)
    .option("--dry-run", "只打印请求体")
  ...
  envCmd.command("metrics")
    .description("CPU/内存/GPU 利用率（支持数字 ID 自动解析 uuid）")
    .argument("<idOrUuid>", "开发机数字 ID 或资源实例 UUID")
    .option("--json", "JSON 输出")
    .option("--minutes <n>", "时间窗（官方通道，默认 10，需 5-1440）", intArg)
    .action(envMetricsAction);

  envCmd.command("exec")
    .description("经 Jupyter kernel 执行代码（免 SSH）")
    .argument("<ideId>", "开发机 ID", intArg)
    .argument("[spec...]", "python代码 | 文件.py | cmd:shell命令")
    .action(envExecAction);

  envCmd.command("ssh-setup")
    .description("注入公钥到 authorized_keys（幂等）")
    .argument("<ideId>", "开发机 ID", intArg)
    .option("--key <path>", "公钥文件路径（默认 ~/.ssh/id_ed25519.pub）")
    .action(envSshSetupAction);

  envCmd.command("ssh-ips")
    .description("更新开发机 SSH 白名单（须运行中且已开 SSH）")
    .argument("<ideId>", "开发机 ID", intArg)
    .requiredOption("--ips <ips>", "CIDR 逗号分隔 (如 1.2.3.4/32,5.6.7.8/32)")
    .action(envSshIpsAction);
  ...
  const HELP_GROUPS = [
    ...
    ["镜像", ["image"]],
    ["SSH 公钥", ["key"]],
    ["队列与资源池", ["queue", "pool", "spec", "quota", "bill"]],
    ["监控与审计", ["summary", "events", "whoami", "my-ip"]],
    ["逃生舱", ["api", "raw"]],
  ];
